...
from poker_enums import HandStrength, Rank, Suit, Card
...
class HandEvaluator:
    ...
    def evaluate_hand_strength(self, hole_cards: List[str], board: List[str]) -> HandStrength:
        ...
        if board:
            ...
            if len(board) == 3:
                ...
                street = Street.RIVER
            self.hand_strength_symbols.update_cards(hole_cards, board, street)
        ...
        if made_hand.value >= HandStrength.TWO_PAIR_BOTTOM.value:
            # Check for redraws
            if self.hand_strength_symbols.have_flush_redraw():
                if made_hand.value >= HandStrength.SET.value:
                    return HandStrength.SET_WITH_FLUSH_REDRAW
                else:
                    return HandStrength.PAIR_WITH_FLUSH_REDRAW
            elif self.hand_strength_symbols.have_straight_redraw():
                if made_hand.value >= HandStrength.SET.value:
                    return HandStrength.SET_WITH_STRAIGHT_REDRAW
                else:
                    return HandStrength.PAIR_WITH_STRAIGHT_REDRAW
            return made_hand

        # Drawing hands
        draw_hand = self._evaluate_draws(hole_cards, board)

        # Check for blockers in drawing hands
        if draw_hand and (self.hand_strength_symbols.have_ace_blocker() or
                         self.hand_strength_symbols.have_king_blocker() or
                         self.hand_strength_symbols.have_flush_blocker() or
                         self.hand_strength_symbols.have_straight_blocker()):
            if draw_hand.value >= HandStrength.FLUSH_DRAW.value:
                return HandStrength.SEMI_BLUFF_WITH_BLOCKER
            else:
                return HandStrength.BLUFF_WITH_BLOCKER
    ...
    def _evaluate_made_hand(self, hole_cards: List[str], board: List[str]) -> HandStrength:
        ...
        # Three of a kind
        if score >= 3000000:
            if self.hand_strength_symbols.have_set():
                if self.hand_strength_symbols.have_top_set():
                    return HandStrength.TOP_SET
                elif self.hand_strength_symbols.have_second_top_set():
                    return HandStrength.SECOND_SET
                return HandStrength.SET
            return HandStrength.TRIPS

        # Two pair
        if score >= 2000000:
            if self.hand_strength_symbols.have_top_two_pair():
            ...
            if self.hand_strength_symbols.have_top_and_bottom_pair():
                return HandStrength.TWO_PAIR_TOP_AND_BOTTOM
            return HandStrength.TWO_PAIR_BOTTOM

        # One pair
        if score >= 1000000:
            if self.hand_strength_symbols.have_overpair():
                if self._rank_to_number(hole_cards[0][0]) >= 12:  # QQ+
                    return HandStrength.OVERPAIR_STRONG
                return HandStrength.OVERPAIR_WEAK

            if self.hand_strength_symbols.have_top_pair():
            ...
            if self.hand_strength_symbols.have_second_top_pair():
                if self._have_good_kicker(hole_cards, board):
                    return HandStrength.SECOND_TOP_PAIR_GOOD_KICKER
                return HandStrength.SECOND_TOP_PAIR_BAD_KICKER

            if self.hand_strength_symbols.have_third_top_pair():
                if self._have_good_kicker(hole_cards, board):
                    return HandStrength.THIRD_TOP_PAIR_GOOD_KICKER
                return HandStrength.THIRD_TOP_PAIR_BAD_KICKER
    ...
    def _evaluate_draws(self, hole_cards: List[str], board: List[str]) -> Optional[HandStrength]:
        ...
        if flush_draw:
            ...
            # Check for second nut flush draw
            suits = [card[1] for card in hole_cards + board]
            suit_counter = Counter(suits)
            flush_suit = suit_counter.most_common(1)[0][0]
            suited_hole_cards = [card for card in hole_cards if card[1] == flush_suit]
            if suited_hole_cards and self._rank_to_number(suited_hole_cards[0][0]) == 13:  # King
                return HandStrength.SECOND_NUT_FLUSH_DRAW
            return HandStrength.FLUSH_DRAW
        ...
        if len(board) == 3:  # Only check backdoor draws on flop
            if self._have_backdoor_flush_draw(hole_cards, board):
                # Check for blockers
                if self.hand_strength_symbols.have_ace_blocker() or self.hand_strength_symbols.have_king_blocker():
                    return HandStrength.BLUFF_WITH_BLOCKER
                return HandStrength.BACKDOOR_FLUSH_DRAW
            if self._have_backdoor_straight_draw(hole_cards, board):
                # Check for blockers
                if self.hand_strength_symbols.have_straight_blocker():
                    return HandStrength.BLUFF_WITH_BLOCKER
    ...
    def _have_overpair(self, hole_cards: tuple, board: list) -> bool:
        """Check if player has a pocket pair higher than any board card"""
        if not board or hole_cards[0][0] != hole_cards[1][0]:
            return False

        pocket_rank = self._rank_to_number(hole_cards[0][0])
        board_ranks = [self._rank_to_number(card[0]) for card in board]

        return pocket_rank > max(board_ranks)
